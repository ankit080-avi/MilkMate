/* MilkMate — single-page dairy manager
...
const Store = {
    ...
    try {
      ...
      (dsRes.data || []).forEach(row => {
        dairySettings[row.ownerId] = {
          ...
          upiId: row.upiId || 'milkmate@upi',
          upiName: row.upiName || 'MilkMate Dairy',
          ownerWhatsApp: row.ownerWhatsApp || '',
          businessName: row.businessName || 'My Dairy',
  ...
  migrate() {
    ...
    if (s) {
      if (s.businessName === undefined) s.businessName = 'My Dairy';
...
async function hashPassword(password, salt) {
  ...
  const bits = await crypto.subtle.deriveBits(
    { name: 'PBKDF2', salt: enc.encode(String(salt || 'milkmate')), iterations: 100000, hash: 'SHA-256' },
    keyMaterial, 256
  );
...
function defaultDairySettings() {
  return {
    ...
    products: {
      ...
      butter: { name: 'Butter (200g)', price: 130, emoji: '🧈', active: true, stock: 0 },
      ghee:   { name: 'Ghee (500ml)',  price: 480, emoji: '🫙', active: true, stock: 0 }
    }
  };
}
...
const I18N = {
  en: {
    app_tagline: 'Daily milk delivery, made simple for owners and customers.',
    ...
    customer: 'Customer', dairy_owner: 'Dairy Owner', delivery_boy: 'Delivery Boy',
    delivery_address: 'Delivery address', daily_quantity: 'Daily milk quantity',
...
function billWhatsAppMessage(customer, bill, monthLabel) {
  const lines = [
    'Hi ' + customer.name + ', your MilkMate bill for ' + monthLabel + ':',
    '',
    '• Milk: ' + fmtQty(bill.totalMl) + ' (' + bill.deliveries + ' days) — ' + fmtMoney(bill.milkAmt),
  ];
...
function viewLogin() {
    ...
    if (state.stage === 'phone') {
      ...
      if (ownerWa) {
        form.appendChild(el('a', {
          ...
          target: '_blank', rel: 'noopener',
          href: waLink(ownerWa,
            'Hi, this is +91 ' + state.mobile + '. I forgot my MilkMate password — please reset it for me.')
        }, t('forgot_password')));
      }
      ...
      form.appendChild(el('div', { class: 'field' }, [
        el('div', { class: 'hint', style: 'margin-bottom:8px' },
          'New dairy owner registration. After signup, the software admin will review and approve your account.')
      ]));
      form.appendChild(el('div', { class: 'field' }, [
      ...
      form.appendChild(el('div', { class: 'field' }, [
        el('label', {}, 'Dairy / business name'),
        el('input', { class: 'input', id: 'su-biz', type: 'text', placeholder: 'e.g. Patel Dairy' })
      ]));
      form.appendChild(el('div', { class: 'field' }, [
      ...
      form.appendChild(el('a', {
        ...
        href: waLink('918858141463', 'Hi, I applied for a MilkMate owner account (' + state.mobile + '). Please review my request.'),
        target: '_blank', rel: 'noopener'
      }, 'WhatsApp admin'));
      form.appendChild(el('button', {
        class: 'link-btn', type: 'button', style: 'margin-top:12px',
      ...
      form.appendChild(el('a', {
        class: 'btn btn-primary btn-block', style: 'margin-top:14px',
        href: waLink('918858141463', 'Hi, my MilkMate owner application (' + state.mobile + ') was rejected. Could you reconsider?'),
        target: '_blank', rel: 'noopener'
      }, 'WhatsApp admin'));
      form.appendChild(el('button', {
        class: 'link-btn', type: 'button', style: 'margin-top:12px',
  ...
  const next = async () => {
    if (state.stage === 'phone') {
      ...
      if (!biz)  return toast('Enter your dairy / business name', 'error');
...
function adminOwnerDetail(ownerId) {
  ...
  if (owner.status === 'pending') {
    ...
    wrap.appendChild(el('a', {
      class: 'btn btn-ghost btn-block', style: 'margin-top:8px',
      href: waLink(owner.mobile, 'Hi ' + owner.name + ', this is the MilkMate admin.'),
      target: '_blank', rel: 'noopener'
    }, 'WhatsApp owner'));
    wrap.appendChild(el('button', {
...
function adminOwnerCustomers(ownerId) {
  ...
  if (customers.length === 0 && boys.length === 0) {
    page.appendChild(emptyState('👥', 'No users yet', 'This dairy hasn\'t added any customers or delivery boys.'));
  }
...
function billDetail(customerId, month) {
  ...
  wrap.appendChild(el('div', { class: 'bill-rows' }, [
    el('div', { class: 'bill-row' }, [
      el('span', {}, 'Milk delivered'),
      el('span', { class: 'text-mono' }, fmtQty(b.totalMl) + ' · ' + b.deliveries + ' days')
    ]),
    el('div', { class: 'bill-row' }, [
      el('span', {}, 'Milk amount @ ' + fmtMoney(Store.data.settings.pricePerLitre) + '/L'),
...
function ownerReportDetail(kind) {
  ...
  page.appendChild(el('div', { class: 'print-only print-header' }, [
    el('h1', {}, 'MilkMate — ' + (titles[kind] || 'Report')),
...
function productForm(key) {
  ...
  wrap.appendChild(el('div', { class: 'field' }, [
    el('label', {}, 'Name'),
    el('input', { class: 'input', id: 'pf-name', type: 'text', value: p.name, autofocus: true, placeholder: 'e.g. Paneer (250g)' })
  ]));
  wrap.appendChild(el('div', { class: 'field' }, [
    el('label', {}, 'Price (₹)'),
...
function ownerProcurementDetail() {
  ...
  const page = el('div', { class: 'page printable' });
  page.appendChild(el('div', { class: 'print-only print-header' }, [
    el('h1', {}, 'MilkMate — Procurement plan'),
  ...
  if (Object.keys(buckets).length) {
    ...
    page.appendChild(reportTable(['Per customer', 'Customers', 'Total milk'],
...
function ownerStatDetail(kind) {
  ...
  if (kind === 'today') {
    ...
    page.appendChild(reportTable(['#', 'Customer', 'Days', 'Volume', 'Milk', 'Extras', 'Total', 'Due'],
      bills.map((x, i) => ({
        cells: [
...
function generateBillPDF(customer, bill, monthIso) {
  ...
  doc.setFontSize(18);
  doc.text(s.businessName || 'MilkMate Dairy', margin, 11);
  ...
  doc.text('Generated by MilkMate', pageW - margin, footerY + 11, { align: 'right' });
...
function renderBillPreviewHtml(customer, bill, monthIso) {
  ...
  header.appendChild(el('div', { class: 'bill-business-name' }, s.businessName || 'MilkMate Dairy'));
  ...
  footer.appendChild(el('div', { class: 'bill-brand' }, 'Generated by MilkMate'));
  ...
  actions.appendChild(el('button', {
    ...
    onclick: async () => {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({ files: [file], title: 'MilkMate Bill', text: message });
...
function customerBill() {
  ...
  page.appendChild(el('div', { class: 'card' }, [
    el('div', { class: 'bill-rows' }, [
      el('div', { class: 'bill-row' }, [el('span', {}, 'Milk delivered'), el('span', { class: 'text-mono' }, fmtQty(b.totalMl))]),
      el('div', { class: 'bill-row' }, [el('span', {}, 'Milk amount @ ' + fmtMoney(Store.data.settings.pricePerLitre) + '/L'), el('span', { class: 'text-mono' }, fmtMoney(b.milkAmt))]),
      ...b.extras.map(o => el('div', { class: 'bill-row' }, [
        el('span', {}, products[o.productKey].name + ' × ' + o.qty),
        el('span', { class: 'text-mono' }, fmtMoney(products[o.productKey].price * o.qty))
      ])),
...
function payViaUPI(amount) {
  const link = upiDeepLink(amount, 'MilkMate ' + monthKey());
  const img = makeQRImage(link);
  const wrap = el('div', {});
...
function customerExtras() {
  $view.appendChild(topbar({ title: 'Order extras', subtitle: 'Paneer, curd, butter & more', bell: true }));
  const page = el('div', { class: 'page' });
  const products = Store.data.settings.products;
...
function navigate(route) {
  ...
  if (route === 'login') {
    ...
    if (dairy && ownerSubscriptionState(dairy) === 'expired') {
      toast('Service expired — contact your dairy owner', 'error');
